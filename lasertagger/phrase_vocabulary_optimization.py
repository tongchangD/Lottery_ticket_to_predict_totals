# 运行参数 --input_file=./data/train.txt --input_format=wikisplit --vocabulary_size=500 --max_input_examples=1000000  --enable_swap_tag=false --output_file=./output/label_map.txt
...
def _get_added_phrases(source: Text, target: Text) -> Sequence[Text]:
    ...
    source_tokens = utils.get_token_list(source.lower())  # list(source.lower()) #  切句成字列表
    target_tokens = utils.get_token_list(target.lower())  # list(target.lower()) #  切句成字列表
    ...
    with tf.io.gfile.GFile(FLAGS.output_file, 'w') as writer:
        with tf.io.gfile.GFile(statistics_file, 'w') as stats_writer:
            ...
            for i, (phrase, count) in enumerate(
                ...
                # Write tags.
                if i < FLAGS.vocabulary_size:  # TODO  为什么要限制一个phrase既能在KEEP前，也能在DELETE前？？　
                    writer.write(f'KEEP|{phrase}\n')
                    writer.write(f'DELETE|{phrase}\n')
                # Write statistics.
                coverage = 100.0 * _count_covered_examples(matrix, i + 1) / num_examples  # 用前i+1个高频phrase能覆盖的语料的比例
