...
class BertExampleBuilder(object):
    ...
    def build_bert_example(
        ...
        #print("bert example.py sources",sources) #
        task = tagging.EditingTask(sources, location=location)
        #print("task",task)
        #print("source_tokens", task.source_tokens)
        # print("task.target_tokens",task.target_tokens)
        if target is not None:
            ...
            if not tags:  # 不可转化，取决于　use_arbitrary_target_ids_for_infeasible_examples
                if use_arbitrary_target_ids_for_infeasible_examples:
                    ...
                    #print("[tagging.Tag('KEEP') 。。。]",[tagging.Tag('KEEP') if i % 2 == 0 else tagging.Tag('DELETE') for i, _ in enumerate(task.source_tokens)])
                else:
                    return None
        else:
